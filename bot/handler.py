...
from bot.BotClasses import command_list, Message, User
...
async def message_handler(update, tg_client, debug=False):
    ...
    for c in command_list:
        ...
        if message.callback_query_id:
            if message.button in c.payload:
        ...
        for k in c.keys:
            ...
            if d < distance:
                ...
                command = c
                key = k
                if distance == 0 and c.admlevel <= user.admLevel:
                    await c.process(user, message, tg_client, stage=stage)
                    return
    if distance < len(
            message.text.lower()) * 0.4 and command.admlevel <= user.admLevel and \
            message.text[1] != '/':
...
async def check_subscription(tg_client, user_id):
    try:
        result = await tg_client.get_chat_member(user_id)
